    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-brand-600 to-brand-800 rounded-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-brand-100 mt-1">Sistem Informasi Manajemen - BPS Lombok Tengah</p>
          ...
          <div className="text-right">
            <div className="text-3xl font-bold">{formatNumber(statsData.totalKegiatan + statsData.totalMitra)}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Jumlah Kegiatan</p>
              <p className="text-2xl font-bold text-gray-900">{formatNumber(statsData.totalKegiatan)}</p>
            </div>
          ...
          <div className="mt-4 flex items-center">
            ...
            <span className="text-sm text-green-600 font-medium">+{statsData.kegiatanTrend}%</span>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{formatNumber(statsData.totalMitra)}</p>
            </div>
          ...
          <div className="mt-4 flex items-center">
            <TrendingUp className="h-4 w-4 text-green-500 mr-1" />
            <span className="text-sm text-green-600 font-medium">+{statsData.mitraTrend}%</span>
            <span className="text-sm text-gray-500 ml-2">dari bulan lalu</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{formatCurrency(statsData.totalHonor)}</p>
            </div>
          ...
          <div className="mt-4 flex items-center">
            <TrendingUp className="h-4 w-4 text-green-500 mr-1" />
            <span className="text-sm text-green-600 font-medium">+{statsData.honorTrend}%</span>
            <span className="text-sm text-gray-500 ml-2">dari bulan lalu</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Monthly Honor Trend */}
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Tren Honor Bulanan</h3>
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={monthlyHonorData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="name" stroke="#64748b" fontSize={12} />
              <YAxis stroke="#64748b" fontSize={12} tickFormatter={(value) => formatCurrency(value).slice(0, -3) + 'M'} />
              <Tooltip 
                formatter={(value: number) => [formatCurrency(value), 'Honor']}
                labelStyle={{ color: '#374151' }}
              />
              <Area type="monotone" dataKey="honor" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Team Distribution */}
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Distribusi Kegiatan Per Tim</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">10 Teratas Honor Mitra Per Bulan</h3>
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{mitra.kegiatan} kegiatan</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-gray-900">{formatCurrency(mitra.honor)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Honor by Category */}
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Honor Per Kategori</h3>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={honorByCategoryData} layout="horizontal">
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis type="number" stroke="#64748b" fontSize={10} tickFormatter={(value) => formatCurrency(value).slice(0, -6) + 'M'} />
              <YAxis type="category" dataKey="name" stroke="#64748b" fontSize={10} width={80} />
              <Tooltip formatter={(value: number) => [formatCurrency(value), 'Honor']} />
