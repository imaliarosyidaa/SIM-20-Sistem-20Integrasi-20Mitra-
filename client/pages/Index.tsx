  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        {batasHonor.map((honor, index) => (
          <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg p-6 hover:shadow-3xl transition-all duration-300 hover:scale-105">
            <div className="text-white">
              <h3 className="text-xl font-semibold mb-2">Rp{honor.biaya.toLocaleString('id-ID')}</h3>
              <p className="text-white/90">{honor.nama_posisi}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        {files.map((file, index) => (
          <a className="relative bg-gray-900 block p-6 border border-gray-100 rounded-lg mx-auto hover:shadow-3xl transition-all duration-300 hover:scale-105">
            ...
            <div className="my-4">
              <h2 className="text-white text-2xl font-bold pb-2">{file.description}</h2>
              <p className="text-gray-300 py-1">{file.originalName}</p>
            </div>
